...
static void event_handler(lv_event_t * e) {
    ...
    if (buttonState) {
        ...
        lv_label_set_text(button_label, "Attack");
        // 执行 stop 相关的操作
        running = 0;
        delay(delayMilliseconds); // 延迟
        // 在休眠的时候关灯
...
void my_lvgl_interface(void) {
    ...
    lv_obj_align(switch1, LV_ALIGN_TOP_MID, -60, 30);
    ...
    lv_obj_align(switch2, LV_ALIGN_TOP_MID, 0, 30);
    ...
    // switch3 默认关闭
    lv_obj_align(switch3, LV_ALIGN_TOP_MID, 60, 30);
